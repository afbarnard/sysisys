...
# Treat files as mappings of keys to lists of values.
...
function intersect() {
    ...
    # Combine sets and yield duplicates.  Make sure the sets have
    # unique items first.
    for fileset; do
...
# Map CLI operations to functions
declare -A op_map
op_map=(
    [add]=add
    [del]=delete
